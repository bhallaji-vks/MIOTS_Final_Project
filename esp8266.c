...
//#define SSID_NAME  "JoeyiPhone"
//#define PASSKEY    "54h55ft2vn6wm"
//#define SSID_NAME  "JillJungJuk"
//#define PASSKEY    "JalsaPannungada"
//#define SSID_NAME  "JoeyiPhone"
//#define PASSKEY    "54h55ft2vn6wm"
//#define SSID_NAME  "Bhallaji's iPhone"
//#define PASSKEY    "12345678"
//#define SSID_NAME  "Virag"
//#define PASSKEY    "12345678"

#define SSID_NAME  "Cantconnect"
#define PASSKEY    "Sahana123"
//#define SEC_TYPE   ESP8266_ENCRYPT_MODE_WPA2_PSK

#define BUFFER_SIZE 1024
#define MAXTRY 10
#define DAC_FIFOSIZE 16384
...
char temp_buffer_get[2];
//char buffer_get [1464];
int tester = 9000;
#define TESTER_SIZE 8192
uint32_t Rx_DACPutI;
uint32_t Rx_DACGetI;
char  Rx_DACFifo[DAC_FIFOSIZE];
int i;
...
void Rx_DACFifo_Init(void)
{
  long sr;
  sr = StartCritical();
  Rx_DACPutI = Rx_DACGetI = 0;
  EndCritical(sr);
}

int Rx_DACFifo_Put(char data)
{
	long sr;
	sr = StartCritical();
  if((Rx_DACPutI-Rx_DACGetI) & ~(DAC_FIFOSIZE - 1))
  {
    return (FIFOFAIL);
  }
  Rx_DACFifo[Rx_DACPutI &(DAC_FIFOSIZE-1)] = data;
  Rx_DACPutI++;
  EndCritical(sr);
  return(FIFOSUCCESS);
}

 int Rx_DACFifo_Get(char *datapt)
{
if(Rx_DACPutI == Rx_DACGetI)
{
return(FIFOFAIL);
}
*datapt = Rx_DACFifo[Rx_DACGetI &(DAC_FIFOSIZE-1)];
Rx_DACGetI++;
return(FIFOSUCCESS);
}

uint32_t Rx_DACFifo_Size(void)
{
return ((uint32_t)(Rx_DACPutI - Rx_DACGetI));
}
//-------------------SearchCheck -------------------
// - start looking for string in received data stream
// Inputs: none
...
void ESP8266_InitUART(uint32_t baud, int echo){ volatile int delay;
 ...
 //   UART1_IBRD_R = 2;       // IBRD = int(80,000,000 / (16 * 115,200)) = int(43.403)
  //  UART1_FBRD_R = 32;       // FBRD = round(0.4028 * 64 ) = 26
...
uint32_t RX_SearchIPD_State = 0;
uint32_t RX_State = 0;
uint32_t RX_Ignore_Counter = 0;
/**
 *	Called once ESP initalization is done. Called from interrupt context
 */
void ESP8266FIFOtoBuffer_DAC(void)
{
	...
	while((UART1_FR_R&UART_FR_RXFE) == 0)
	{
		 ...
		 * State 0: ignore everything until we see +ipd
		 * State 1: ignore next 6 bytes which will be ',', a four digit size, and a ':' 
		 * State 2: Start receiving data until '\n' and putting into buffer
		 * 
		 */
			...
			case 0:
				switch(RX_SearchIPD_State){
					case 0:
						...
						break;
					case 1:
						//We have a +, not the next must be an 'I'
						if (letter != 'I'){
							RX_SearchIPD_State = 0;
						}else{
							RX_SearchIPD_State++;
						}
						break;
					...
					case 3:
						//Find a D
						if (letter == 'D'){
							//INdicates we have found "+IPD" advance to the next RX_state
							RX_State = 1;
						}
						else{
							RX_SearchIPD_State = 0;
				...
				break;
			//Ignore the next 6 characters
			case 1:
				RX_Ignore_Counter++;
				if(RX_Ignore_Counter >=6){
					RX_Ignore_Counter = 0;
					RX_State = 2;
				}
				break;
			case 2:
				if (letter == 'z'){	//z is our end sentinel
					RX_State = 0;	//Reset state machine
					ESP_Init_Done = false;
				}else{
					Rx_DACFifo_Put(letter);
				}
				break;
		}
...
//		if(ESP8266_EchoResponse)
//		{
//			UART_OutCharNonBlock(letter);
//		}
		
		/**
		temp_buffer_get[i] = letter;
		//+:2B.I=49,P=50,D=44
		if(i!=0)
		{
			if(temp_buffer_get[i-1] == '+' && temp_buffer_get[i] == 'I')
			{
				tester = TESTER_SIZE;
				//tester = 95;
			}
		...
		if(tester <= TESTER_SIZE && tester >= 0)
		{
			Rx_DACFifo_Put(letter);
		//	buffer_get[TESTER_SIZE-tester] = letter;
			tester--;
		}
		EndCritical(sr);
			
		
		if(buffer_get[i] == 0x22)
		{ 
			tester = 1;
			//buffer_get[i] = letter;
			//i++;
	...
	*/
		...
		//FIXME
		//*********IMPORTANT*****************ESP_Init_Done = false;	//Start a new search
...
//				}
//			}
//			else if(letter-1 != prev)
//			{	//printf("E");
//				UART_OutCharNonBlock(letter);
//				//UART_OutCharNonBlock(prev);
//				GPIO_PORTF_DATA_R ^= 0x08; 
//			}
//			prev = letter;
//		}
//		else if(ESP8266_EchoResponse)
//		{
//			UART_OutCharNonBlock(letter);
//		}
//		 
...
//{
	...
	ESP8266_Reset();
	ESP8266SendCommand("AT+UART_DEF=2000000,8,1,0,0\r\n");
//ESP8266_Init(2000000); 
	ESP8266_InitUART(2000000,true);
	ESP8266_Reset();
//	}
  ...
  //}
 //ESP8266SendCommand("AT+UART_CUR=2000000,8,1,0,0\r\n");
 //UART_InChar();
//ESP8266_Reset();
//  ESP8266_InitUART(115200,true);
  
// step 2: AT+CWMODE=1 set wifi mode to client (not an access point)
ESP8266_SetWifiMode(ESP8266_WIFI_MODE_CLIENT);
...
//  while(try){
	...
	//sprintf((char*)TXBuffer, "AT+CIPSTART=\"TCP\",\"%s\",81\r\n", IPaddress);
	//sprintf((char*)TXBuffer, "AT+CIPSTART=\"TCP\",\"%s\",80\r\n", IPaddress);
